...
euler8Tail number windowSize =
  ...
  where
    ...
    maxProduct xs currentMax
      ...
      | otherwise =
          let windowProduct = product (take windowSize xs)
              newMax = max currentMax windowProduct
           in maxProduct (tail xs) newMax
...
euler8Recursion number windowSize =
  ...
  where
    ...
    maxProduct xs
      ...
      | otherwise = max windowProduct restMax
      where
...
euler8Modular number windowSize =
  let digits = map digitToInt (show number)
   ...
   in -- maxProduct = foldl (\acc x -> max acc (product x)) 0 validWindows
